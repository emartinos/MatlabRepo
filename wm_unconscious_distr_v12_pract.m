...
if data.subject == 999 || data.subject == 998
    ...
    resultFileName = ['s', int2str(data.subject) '_session' int2str(data.session) , '_v12_pract.mat' ]; % Creates result file using subject ID and run number.
...
data.Ntrials  = 24;%length(data.conds);
data.Nblocks  = 1;%size(data.conds,3);
% data.trial    = [0];
data.MemSet   = [];
data.output   = [];
% =================
